...
class WordMath():
    ...
    >>> wm.compute()
    33
    >>> WordMath('nine hundred ninety nine divided by nine divided by three').compute()
    37
    """
    ...
    hundreds_words = 'one hundred, two hundred, three hundred, four hundred, five hundred, six hundred, ' \
                     'seven hundred, eight hundred, nine hundred'.split(', ')
    number_words = digit_words + tens_words + decades_words + hundreds_words

    operator_words = {'add': '+', 'plus': '+', 'minus': '-', 'subtract': '-',
                      'times': '*', 'multiplied by': '*', 'divide': '/', 'divided by': '/', 'exponent': '**',
                      'power of': '**', 'modulo': '%'}
    ...
    def symbolize_one_token(token: str) -> str:
        ...
        5
        >>> WordMath.symbolize('fifty')
        50
        >>> WordMath.symbolize('nine hundred')
        900
        """
        ...
        elif token in WordMath.hundreds_words:
            return (WordMath.hundreds_words.index(token) + 1) * 100 # calculate hundreds words
    ...
    def symbolize(tokens):
        ...
        [3, '*', 5]
        >>> WordMath.symbolize(['add', 'plus', 'minus', 'subtract', 'times', 'divide', 'exponent', 'power of', 'modulo'])
        ['+', '+', '-', '-', '*', '/', '**', '**', '%']
        >>> WordMath.symbolize('multiplied by')
        '*'
        >>> WordMath.symbolize(3)
    ...
    def tokenize(s: str) -> list:
        ...
        >>> WordMath.tokenize('subtract five from nine')
        Traceback (most recent call last):
        ...
        ValueError
        """
        ...
        while len(s):
            ...
            if result:
                fw = result.group(1)
                if fw in WordMath.number_words or fw in WordMath.operator_words:
    ...
    def compute(self):
        ...
        >>> WordMath('plus five').compute()
        Traceback (most recent call last):
        ...
        ValueError
        >>> WordMath('five minus plus five').compute()
        Traceback (most recent call last):
        ...
        ValueError
        >>> WordMath('nine hundred ninety nine minus three').compute()
        996
        """
